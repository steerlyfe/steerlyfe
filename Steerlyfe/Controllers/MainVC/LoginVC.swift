...
class LoginVC: UIViewController, UITextViewDelegate, CountrySelectionDelegate, UITextFieldDelegate, OtpSendDelegate {
    ...
    override func viewDidLoad() {
        ...
        CommonMethods.common.showLog(tag: TAG, message: "shortCode : \(shortCode)")
        if MyConstants.FILL_STATIC_FORM{
            shortCode = "IN"
            phoneNumber.text = "8699249405"
        }
        if let countryDetail = databaseMethods.getCurrentCountry(shortCode: shortCode){
            onCountrySelected(countryDetail: countryDetail)
        }else{
            CommonMethods.common.showLog(tag: TAG, message: "Country Detail not available")
            countryCodeLabel.text = ""
        }
        phoneNumber.delegate = self
//        let verificationID = UserDefaults.standard.string(forKey: MyConstants.AUTH_VERIFICATION_ID)
//        if verificationID != ""{
//            MyNavigations.navigation.goToVerifyOtp(navigationController: self.navigationController, phoneNumber: phoneNumber)
//        }
    }
    ...
    @IBAction func submitButtonPressed(_ sender: Any) {
        if self.countryDetail != nil{
            if let phone = phoneNumber.text?.trimmingCharacters(in: .whitespaces){
                if phone.count >= 10{
                    let value = "+\(countryDetail?.callingCode ?? "")\(phone)"
//                    MyNavigations.navigation.goToSignup(navigationController: navigationController, phoneNumber: value)
//                    MyNavigations.navigation.goToVerifyOtp(navigationController: self.navigationController, phoneNumber: value, verificationID: "1223")
                    showChooserOption(phoneNumber: value)
                }else{
                    MyNavigations.navigation.showCommonMessageDialog(message: "Enter valid phone number", buttonTitle: "OK")
                }
            }else{
                MyNavigations.navigation.showCommonMessageDialog(message: "Enter phone number", buttonTitle: "OK")
            }
        }else{
            MyNavigations.navigation.showCommonMessageDialog(message: "Choose country code", buttonTitle: "OK")
        }
    }
    
    private func setupUI() {
        submitButton.setTitleColor(UIColor.white, for: .normal)
        CommonMethods.common.addRoundCornerStroke(uiview: countryCodeView, borderWidth: 1.0, borderColor: UIColor.colorPrimaryDark, cornerRadius: 5.0)
        CommonMethods.common.addRoundCornerStroke(uiview: phoneNumberView, borderWidth: 1.0, borderColor: UIColor.colorPrimaryDark, cornerRadius: 5.0)
        CommonMethods.common.addRoundCornerFilled(uiview: submitButton, borderWidth: 1.0, borderColor: UIColor.colorPrimaryDark, backgroundColor: UIColor.colorPrimaryDark, cornerRadius: 20.0)
...
//    }
    ...
    @IBAction func chooseCountryPressed(_ sender: Any) {
        MyNavigations.navigation.goToChooseCountry(navigationController: navigationController, pageTitle: "Choose Calling Code", countrySelectionDelegate: self)
    }
    ...
    func showChooserOption(phoneNumber : String) {
    ...
    func onOtpSend(phoneNumber: String, verificationID: String) {
        MyNavigations.navigation.goToVerifyOtp(navigationController: self.navigationController, phoneNumber: phoneNumber, verificationID: verificationID)
    }    
}
